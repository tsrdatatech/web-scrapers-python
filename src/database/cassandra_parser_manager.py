...
class CassandraParserManager(BaseParserManager):
    ...
    async def initialize(self) -> None:
        ...
        try:
            ...
            logger.warning(
                "Failed to initialize Cassandra, running without database",
                error=str(e)
            )
    ...
    async def add_seed_url(self, url: str, label: Optional[str] = None, 
                          parser: Optional[str] = None, priority: int = 1) -> None:
...
async def create_cassandra_parser_manager(registry, cassandra_config: Optional[CassandraConfig] = None):
