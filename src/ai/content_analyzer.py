...
class ContentAnalysis(BaseModel):
    ...
    sentiment: str = Field(..., description="Article sentiment: positive, negative, neutral, mixed")
    confidence: float = Field(..., ge=0.0, le=1.0, description="Analysis confidence score")
    
    # Content Quality
    quality_score: float = Field(..., ge=0.0, le=10.0, description="Content quality score (0-10)")
    readability: str = Field(..., description="Readability level: basic, intermediate, advanced")
    completeness: float = Field(..., ge=0.0, le=1.0, description="Content completeness score")
    
    # Classification  
    topics: List[str] = Field(default_factory=list, description="Main article topics/categories")
    entities: List[str] = Field(default_factory=list, description="Key entities (people, orgs, places)")
...
class AdvancedContentAnalyzer:
    ...
    async def _run_llm_analysis(self, title: str, content: str) -> Dict[str, Any]:
        ...
        try:
            ...
            summary = await summary_chain.ainvoke({"title": title, "content": content_sample})
            
            # Get sentiment
            sentiment_chain = self.sentiment_prompt | self.llm  
            sentiment = await sentiment_chain.ainvoke({"title": title, "content": content_sample})
            
            # Get topics
            topics_chain = self.topics_prompt | self.llm
            topics_str = await topics_chain.ainvoke({"title": title, "content": content_sample})
    ...
    def _fallback_analysis(self, title: str, content: str, word_count: int) -> ContentAnalysis:
    ...
    def _calculate_quality_score(self, title: str, content: str, word_count: int) -> float:
        ...
        if content:
            ...
            if '?' in content or '!' in content:  # Engagement indicators
                score += 0.3
            if any(word in content.lower() for word in ['according to', 'research', 'study', 'data']):
    ...
    def _basic_sentiment_analysis(self, content: str) -> str:
        ...
        positive_words = ['good', 'great', 'excellent', 'amazing', 'breakthrough', 'success', 'achievement']
        negative_words = ['bad', 'terrible', 'crisis', 'problem', 'issue', 'concern', 'failure']
    ...
    def _extract_topics_basic(self, text: str) -> List[str]:
        ...
        topic_keywords = {
            "technology": ["tech", "digital", "software", "computer", "ai", "artificial intelligence"],
            "business": ["business", "company", "market", "economy", "finance", "startup"],
            "health": ["health", "medical", "hospital", "doctor", "medicine", "treatment"],
            "politics": ["government", "political", "policy", "election", "president", "minister"],
            "science": ["research", "study", "scientist", "discovery", "experiment", "analysis"],
