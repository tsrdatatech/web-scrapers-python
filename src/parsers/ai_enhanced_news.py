...
class AIEnhancedNewsParser(GenericNewsParser):
    ...
    def __init__(self, enable_ai_analysis: bool = True):
        ...
        self.content_analyzer = create_content_analyzer(use_mock_llm=True) if enable_ai_analysis else None
        ...
        try:
            ...
            if self.enable_ai_analysis and self.content_analyzer:
                try:
                    ...
                    logger.warning(f"AI analysis failed, returning standard article: {ai_error}")
                    return article
            
            return article
            
        except Exception as e:
            logger.error(f"AI-enhanced parsing failed for {context.get('request', {}).get('url', 'unknown')}: {e}")
            return None
