...
def build_router(registry: ParserRegistry, manager: ParserManager) -> Router:
    ...
    async def default_handler(context) -> None:
        ...
        if not selector:
            log.warning(f"No selector provided for {request.url} - skipping link discovery")
            return
        ...
        try:
            await enqueue_links(
                ...
                transform_request_function=lambda req: {
                    **req,
                    "user_data": {"parser": request.user_data.get("parser") if request.user_data else None}
                }
            )
    ...
    async def parse_handler(context) -> None:
        ...
        forced_parser_id = request.user_data.get("parser") if request.user_data else None
        ...
        try:
            ...
            if result:
                ...
                data_logger.info("Successfully parsed content", extra={"data": result.dict()})
            else:
