...
class TestCassandraManager:
    ...
    @pytest.mark.asyncio 
    async def test_cassandra_manager_initialization(self, cassandra_manager, cassandra_config):
    ...
    @pytest.mark.asyncio
    async def test_store_article_success(self, cassandra_manager, mock_cluster, mock_session, sample_article):
        ...
        with patch('src.database.cassandra_manager.Cluster', return_value=mock_cluster):
            ...
            result = await cassandra_manager.store_article(sample_article, "generic_news")
            
            # Verify article was stored (not duplicate)
            assert result is True
            
            # Verify database calls were made
            assert mock_session.execute.call_count >= 3  # check_url, insert_article, insert_url_tracker

    @pytest.mark.asyncio
    async def test_store_article_duplicate(self, cassandra_manager, mock_cluster, mock_session, sample_article):
    ...
    async def test_get_seed_urls(self, cassandra_manager, mock_cluster, mock_session):
        ...
        with patch('src.database.cassandra_manager.Cluster', return_value=mock_cluster):
            ...
            mock_prepared = Mock()
            cassandra_manager._prepared_statements = {
                'get_active_seeds': mock_prepared
            }
            
            # Mock seed data
            mock_rows = [
                Mock(url="https://example.com/news", label="a.article-link", parser="news"),
                Mock(url="https://techcrunch.com", label="h2 a", parser="news"),
                Mock(url="https://reddit.com/r/tech", label="a[data-click-id]", parser="generic_news")
            ]
    ...
    @pytest.mark.asyncio
    async def test_get_crawl_statistics(self, cassandra_manager, mock_cluster, mock_session):
    ...
    async def test_connection_error_handling(self, cassandra_config):
        ...
        with patch('src.database.cassandra_manager.Cluster', side_effect=Exception("Connection failed")):
    ...
    async def test_create_cassandra_manager_factory(self):
        ...
        with patch('src.database.cassandra_manager.CassandraManager.connect') as mock_connect:
